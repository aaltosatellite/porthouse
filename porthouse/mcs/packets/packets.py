...
def setup_parser(packets_parser: argparse.ArgumentParser) -> None:
    ...
    # Filters
    packets_parser.add_argument('--satellite',
        help="Filter results by the satellite identifier/name")
    packets_parser.add_argument('--type', dest="packet_type",
        help="Filter results by packet type")
    packets_parser.add_argument('--source',
        help="Filter results by packet source")
    packets_parser.add_argument('--after',
        help="Filter results by timestamp")
    packets_parser.add_argument('--before',
        help="Filter results by timestamp")
    ...
    packets_parser.add_argument('--totals', action="store_true",
        help="Get total number of bytes stored in the database")
    packets_parser.add_argument('--export', action="store_true")
    packets_parser.add_argument('--cron', action="store_true")

    packets_parser.add_argument('--routes', action="store_true",
        help="List all PacketRouter's endpoints and routes")
    packets_parser.add_argument('--route', nargs=2,
        help="Create a new route")
    packets_parser.add_argument('--unroute', nargs='*',
        help="Destroy route")



def create_constrain_string(args: argparse.Namespace) -> str:
    """
